...
#Flow statistics
defProperty('pathfile', "/users/content_based_network/rtt.txt", "Path to file")

defApplication('rtt_stats') do |app|
    app.binary_path = '/users/content_based_network/collect.rb'
    app.defProperty('target', 'Address to rtt file', '-f', {:type => :string})  
    app.defMeasurement('wrapper_rtt') do |m|
        m.defMetric(':time_taken', :double)
    end
end

defGroup('Clients', property.c1, property.c2) do |node|
    node.addApplication("rtt_stats") do |app|
        app.setProperty('target', property.pathfile)
        app.measure('wrapper_rtt')
    end
end

#Define graph parameters
defGraph 'RTT' do |g|
    g.ms('wrapper_rtt').select(:time_taken)
